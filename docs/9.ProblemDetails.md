...
This `IPostConfigureOptions` is added by the `AddProblemDetailsDescriptions` method, and the value will be updated only if the `BaseAddress` is the default value.
...
A seguir será apresentado como é feita a conversão de um `ResultMessage` para `ProblemDetails`.
